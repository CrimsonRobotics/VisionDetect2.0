...
public class Utils {
	...
	public static ArrayList sort(ArrayList<MatOfPoint> list){
		boolean run = true;
		MatOfPoint[] array = new MatOfPoint[list.size()-1];
		MatOfPoint temp;
		...
		while(run){
			for(int i = 0; i < array.length-1; i++){
				if(array[i].width() > array[i+1].width()){
					temp = array[i];
					array[i] = array[i+1];
					array[i+1] = temp;
				}
			...
			if(Utils.checkSort(array)){
				run = false;
				ArrayList<MatOfPoint> x = (ArrayList<MatOfPoint>) Arrays.asList(array);
				return x;
			}
	...
	private static boolean checkSort(MatOfPoint[] array){
	...
	public static void printArray(ArrayList<MatOfPoint> array, String name){
		//System.out.print(name + ": ");
		for(int i = array.size()-1; i > 0; i--){
		Rect rect = Imgproc.boundingRect(array.get(i));
		//System.out.print(rect.width +  ", " );
		}
		//System.out.println();
	}
	public static ArrayList<Rect> matOfPointToRect(ArrayList<MatOfPoint> x){
		ArrayList<Rect> temp = new ArrayList<Rect>();
		
		for(int i = 0; i < x.size(); i++){
			temp.add(Imgproc.boundingRect(x.get(i)));
		}
